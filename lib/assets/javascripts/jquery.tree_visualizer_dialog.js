  ...
  $.fn.tree_visualizer_dialog = function(options){
    ...
    if (initialized) {
      ...
      _ = {
        ...
        load: function() {
          $.ajax({
            ...
            success: function(response){
              ...
              $('.spinner', obj).hide()
              var json
              if (json = _.parse(response.documentElement, true, true)){
        ...
        parse: function(node, matchingPath, isLeft) {
          ...
          if (validCildren.length == 2) {
            ...
            ret.actualValue = _.settings.metrics[_.metricsMapping[attribute]]
            var goLeft = null
            ...
            ret.children = $.map(validCildren, function(child, childIndex){
              ...
              if (childIndex == 0)
                return _.parse(child, ret.goLeft && matchingPath, true)
              else
                return _.parse(child, !ret.goLeft && matchingPath, false)
            })
            ...
            return {type: "leaf", error: error, samples: samples, value: value,
              label: decision, claimLabel: _.settings.metrics.label, matchingPath: matchingPath, isLeft: isLeft}
          }
        },
      ...
      function update(source, vis) {
        ...
        linkEnter.append("text")
            .attr("x", function(d) { x= d.source.x0; console.log("x start: ", x); return x})
            .attr("y", function(d) { y= d.source.y0; console.log("y start: ", y); return y})
            .attr("text-anchor", "middle")
            .style("fill-opacity", 1e-6)
            .text(link_label);
        ...
        linkUpdate.selectAll("text")
            .style("fill-opacity", 1)
            .attr("x", function(d) { x= (d.source.x + d.target.x) / 2; console.log("x transition: ", x); return x})
            .attr("y", function(d) { y= (d.source.y + d.target.y) / 2; console.log("y transition: ", y); return y})
        ...
        linkExit.select("text")
            .attr("x", function(d) { x= d.source.x; console.log("x end: ", x); return x})
            .attr("y", function(d) { y= d.source.y; console.log("y end: ", y); return y})
            .style("fill-opacity", 1e-6);
      ...
      // Node labels (html version with unicode marks)
      ...
      function node_label(d, isHtml) {
        if (d.type === "leaf") {
          ...
          return d.label + " [" + d.samples + (d.error > 0 ? ("/" + d.error) : "") + "]" + mark
        } else {
          // split node
